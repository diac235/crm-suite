...
/**
 * Acepta valores booleanos escritos como texto ("true", "1", "sí"…) y también
 * booleanos reales, para que los valores por defecto funcionen cuando la
 * variable no está definida en el archivo .env.
 */
const booleanish = z
...
const schema = z.object({
  ...
  // Útiles en plataformas sin acceso a una terminal en el servidor
  // (Cloud Run, Render): permiten preparar la base durante el arranque.
...
if (!parsed.success) {
  ...
  // Falla temprano: es preferible no arrancar a arrancar mal configurado.
